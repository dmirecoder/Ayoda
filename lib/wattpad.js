/**
 * Author  : Farhannnnn
 * Name    : Wattpad Scrapping 
 * Tools   : Search, Story, Reading 
...
class Wattpad {
	// need a limit to limit the search for data
	constructor(limit) {
		this.limit = limit;
	...
	search(query) {
		return new Promise(async (resolve, reject) => {
			await axios.request({
				method: 'GET',
				url: `${this.baseURL}/v4/search/stories/?query=${encodeURIComponent(query)}&limit=${this.limit}&fields=stories(title%2CvoteCount%2CreadCount%2CcommentCount%2Cdescription%2Ccover%2Curl%2CnumParts%2Cuser(name)%2ClastPublishedPart(createDate)%2Cpromoted%2Csponsor(name%2Cavatar)%2Ctracking(clickUrl%2CimpressionUrl%2CthirdParty(impressionUrls%2CclickUrls))%2Ccontest(endDate%2CctaLabel%2CctaURL))%2Ctotal`,
	...
	story(url) {
		return new Promise(async (resolve, reject) => {
			if (!url.match(/^(?:http?s:\/\/)?(?:www\.|)?(wattpad\.com)?(\/)?(?:story\/)?([\d]+\-)/gi)) throw ('invalid url');
			await axios.request({
				method: 'GET',
				url,
				headers: this.headers
	...
	reading(url) {
		return new Promise(async (resolve, reject) => {
			if (!url.match(/^(?:http?s:\/\/)?(?:www\.|)?(wattpad\.com)?(\/)?([\d]+\-)/gi)) throw ('invalid url');
			const decodeUrl = decodeURIComponent(url);
			const link = new URL(decodeUrl);
			await axios.get(link.href).then(async ({ data }) => {
				const $ = cheerio.load(data);
